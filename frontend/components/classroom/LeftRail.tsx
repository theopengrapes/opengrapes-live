...
export default function LeftRail({ editor, showWhiteboard, strokeWidth = 1.5, isTeacher = false, topPadding, bottomPadding }: LeftRailProps) {
  ...
  // Sync isWhiteboardEmpty state reactively from tldraw store
  useEffect(() => {
    if (!editor) return;

    const checkEmpty = () => {
      const frames = editor.getCurrentPageShapes().filter((s: any) => s.type === 'frame');
      setIsWhiteboardEmpty(frames.length === 0);
    };
  ...
  const handleToolClick = (toolId: string) => {
    ...
    if (toolId === 'draw' || toolId === 'highlight') {
      ...
      setCurrentTool(toolId); // Optimistic Update
      if (isAlreadyActive) {
  ...
  const handleSelectShape = (shapeType: 'rectangle' | 'ellipse' | 'arrow' | 'line') => {
    ...
    if (shapeType === 'rectangle') {
      ...
      setCurrentTool('geo'); // Optimistic Update
      ...
      setCurrentTool('arrow'); // Optimistic Update
    } else if (shapeType === 'line') {
      editor.setCurrentTool('line');
      setCurrentTool('line'); // Optimistic Update
    }
  };
      ...
      {isTeacher && (
        <div className="flex flex-col items-center gap-2.5 border-t border-border/40 pt-4">
          ...
          <div className="relative w-10 h-10 flex items-center justify-center">
            ...
            {/* Onboarding Tooltip pointing at Add Page button */}
            {isWhiteboardEmpty && (
          ...
          <Tooltip content="Zoom to Fit" align="left">
            <button
              ...
              className="w-10 h-10 rounded-lg flex items-center justify-center text-text/70 hover:bg-surface-hi hover:text-text transition-colors cursor-pointer"
            >
          ...
          <div className="relative w-10 h-10 flex items-center justify-center">
            <Tooltip content="Upload PDF/Image" align="left">
              <label className={`w-10 h-10 rounded-lg flex items-center justify-center text-text/70 hover:bg-surface-hi hover:text-text transition-colors select-none ${isImporting ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}>
                {isImporting ? (
            ...
            {/* PDF Import Progress Bar Overlay */}
            {isImporting && importProgressTotal > 0 && (
              <div className="absolute left-14 top-1 z-[120] pointer-events-none whitespace-nowrap animate-in fade-in slide-in-from-left-2 duration-200">
                <div className="flex flex-col gap-1.5 px-4 py-2.5 bg-surface/90 text-text text-xs font-semibold rounded-xl shadow-2xl border border-border/80 backdrop-blur-md w-56 relative">
                  
                  <div className="flex justify-between items-center text-[10px] text-text-muted select-none">
                    <span>IMPORTING PDF</span>
                    <span className="font-mono text-accent">{importProgressCurrent} / {importProgressTotal}</span>
                  </div>
      ...
      {showPenSettings && (
        ...
        >
          ...
          <button
            ...
            className="absolute top-3 right-3 p-1 rounded-md text-text/60 hover:bg-surface-hi hover:text-text transition-colors cursor-pointer"
          ...
          <div className="flex flex-col gap-2">
            <span className="text-[10px] font-bold uppercase tracking-wider text-text/70 select-none">Color</span>
          ...
          <div className="flex flex-col gap-2">
            <span className="text-[10px] font-bold uppercase tracking-wider text-text/70 select-none font-sans">Size</span>
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg flex flex-col items-center justify-center gap-1.5 border text-[10px] font-bold font-sans transition-all cursor-pointer ${
                      activeSize === sz 
                        ...
                        : 'border-border hover:bg-surface-hi text-text/70 hover:text-text'
                    }`}
                  >
      ...
      {showShapesPopover && (
        <div className="absolute left-14 top-[248px] w-48 bg-surface border border-border p-2 rounded-xl shadow-2xl z-30 flex flex-col gap-1 text-text animate-in fade-in slide-in-from-left-2 duration-150">
          <div className="px-2 py-1 select-none border-b border-border/20 mb-1">
            <span className="text-[10px] font-bold uppercase tracking-wider text-text/70">Insert Shape</span>
          </div>
          ...
          >
            <IconSquare className="w-4 h-4 text-text/70" strokeWidth={strokeWidth} />
            <span>Rectangle</span>
          </button>
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-2.5 py-2 rounded-lg hover:bg-surface-hi text-left text-xs font-semibold cursor-pointer text-text"
          >
            <IconCircle className="w-4 h-4 text-text/70" strokeWidth={strokeWidth} />
            <span>Circle</span>
          </button>
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-2.5 py-2 rounded-lg hover:bg-surface-hi text-left text-xs font-semibold cursor-pointer text-text"
          >
            <IconArrowRight className="w-4 h-4 text-text/70" strokeWidth={strokeWidth} />
            <span>Arrow</span>
          </button>
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-2.5 py-2 rounded-lg hover:bg-surface-hi text-left text-xs font-semibold cursor-pointer text-text"
          >
            <IconMinus className="w-4 h-4 text-text/70" strokeWidth={strokeWidth} />
            <span>Line</span>
          </button>
        </div>
      )}
