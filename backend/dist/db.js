...
exports.db = void 0;
exports.insertHandoffCode = insertHandoffCode;
const better_sqlite3_1 = __importDefault(require("better-sqlite3"));
const path_1 = __importDefault(require("path"));
const dbPath = path_1.default.resolve(__dirname, '../opengrapes.db');
...
try {
    exports.db.exec("ALTER TABLE live_sessions ADD COLUMN has_notes INTEGER DEFAULT 0");
}
catch (error) {
...
try {
    exports.db.exec("ALTER TABLE live_sessions ADD COLUMN teacher_joined INTEGER DEFAULT 0");
}
catch (error) {
    // Ignored if column already exists
}
// Add a helper function to write to the handoff table and prune old codes (> 5 min)
function insertHandoffCode(code, userId, role, roomId, batchId) {
    const now = Date.now();
    // Housekeeping: prune old entries
    exports.db.prepare(`DELETE FROM handoff_codes WHERE created_at < ?`).run(now - 5 * 60 * 1000);
    // Insert new code
    exports.db.prepare(`
