...
async function seed() {
    ...
    // 1. Hash passwords
    const teacherPasswordHash = await bcrypt_1.default.hash('teacher123', 10);
    const studentPasswordHash = await bcrypt_1.default.hash('student123', 10);
  ...
  `);
    ...
    const getUserId = (email) => {
        const row = db_1.db.prepare('SELECT id FROM users WHERE email = ?').get(email);
        return row.id;
    };
    ...
    const insertBatch = db_1.db.prepare(`
  ...
  `);
    ...
    // 5. Get batch IDs
    const getBatchId = (name) => {
        const row = db_1.db.prepare('SELECT id FROM batches WHERE name = ?').get(name);
        return row.id;
    };
    ...
    const insertEnrollment = db_1.db.prepare(`
