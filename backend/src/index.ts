...
import { triggerTeacherJoined, triggerMeetingEnded } from './pusher';
...
const getLivekitHttpUrl = (wsUrl: string) => {
  return wsUrl.replace(/^ws(s)?:\/\//, 'http$1://');
};

const roomService = new RoomServiceClient(
  getLivekitHttpUrl(livekitUrl),
  LIVEKIT_API_KEY,
  LIVEKIT_API_SECRET
);

const webhookReceiver = new WebhookReceiver(LIVEKIT_API_KEY, LIVEKIT_API_SECRET);
const teacherLeftTimeouts = new Map<string, NodeJS.Timeout>();
...
const staleSessionTimeouts = new Map<string, NodeJS.Timeout>();
...
function scheduleStaleSessionExpiry(roomId: string, startedAt: Date) {
  const delay = Math.max(STALE_SESSION_MS - (Date.now() - startedAt.getTime()), 0);
  const timeout = setTimeout(async () => {
    staleSessionTimeouts.delete(roomId);
    try {
      const res = await db.query(
        `UPDATE "LiveSession" SET status = 'expired', "endedAt" = NOW() WHERE "roomId" = $1 AND status = 'live'`,
        [roomId]
      );
      if (res.rowCount) {
...
function cancelStaleSessionExpiry(roomId: string) {
...
async function sweepStaleSessionsOnStartup() {
  try {
    const liveRes = await db.query(`SELECT "roomId", "startedAt" FROM "LiveSession" WHERE status = 'live'`);
...
async function isTeacherPresent(roomId: string): Promise<boolean> {
  try {
...
app.post('/api/renew-session', async (req, res) => {
  ...
  try {
    ...
    const sessionRes = await db.query(
      `SELECT status FROM "LiveSession" WHERE "roomId" = $1 AND "startedAt" >= NOW() - INTERVAL '${STALE_SESSION_HOURS} hours'`,
      [decoded.roomId]
    );
...
// POST /api/end-class
app.post('/api/end-class', requireLMSOrClassroomAuth, requireRole('teacher'), async (req, res) => {
  ...
  try {
    ...
    const activeSessionRes = await db.query(
      `SELECT "roomId" FROM "LiveSession" WHERE "batchId" = $1 AND status = 'live' AND "startedAt" >= NOW() - INTERVAL '${STALE_SESSION_HOURS} hours'`,
      [batchId]
    );
    ...
    if (activeSession && activeSession.roomId) {
      ...
      let transcriptData = { segments: [], rollingSummary: '', sessionMeta: {} as any };
      try {
        const doRes = await fetch(`${workerUrl}/api/transcript/${sessionId}/data`, {
          headers: { 'X-Worker-Secret': workerSecret }
        });
        if (doRes.ok) {
          transcriptData = await doRes.json() as any;
        }
      } catch (err) {
      ...
      if (geminiKey && (transcriptData.segments.length > 0 || doubtsList.length > 0)) {
        try {
          const sortedSegments = (transcriptData.segments || []).sort((a: any, b: any) => a.sessionElapsedMs - b.sessionElapsedMs);
          const fullTranscript = sortedSegments.map((s: any) => {
...
Make the tone professional, structured, and easy for students to study from.`;
    ...
    if (activeSession && activeSession.roomId) {
      ...
      await triggerMeetingEnded(activeSession.roomId, batchId, batch.name);
...
app.post('/api/kick-participant', requireClassroomAuth, requireRole('teacher'), async (req, res) => {
...
app.post('/api/mute-participant', requireClassroomAuth, requireRole('teacher'), async (req, res) => {
  ...
  try {
    ...
    const targetType = trackType === 'audio' ? TrackType.AUDIO : TrackType.VIDEO;
...
app.get('/api/turn', async (_req, res) => {
  try {
    ...
    const iceServers = await response.json() as unknown[];
...
async function notifyMeetingEnded(roomId: string) {
  try {
    const sessionRes = await db.query('SELECT "batchId" FROM "LiveSession" WHERE "roomId" = $1', [roomId]);
    const batchId = sessionRes.rows[0]?.batchId;
    if (!batchId) return;
    const batchRes = await db.query('SELECT name FROM "Batch" WHERE id = $1', [batchId]);
    await triggerMeetingEnded(roomId, batchId, batchRes.rows[0]?.name || '');
  } catch (err) {
...
app.post('/api/livekit-webhook', async (req, res) => {
  try {
    ...
    if (event.event === 'room_finished') {
      ...
      const endRes = await db.query('UPDATE "LiveSession" SET status = \'completed\', "endedAt" = NOW() WHERE "roomId" = $1 AND status = \'live\'', [roomName]);
      ...
      if (participantMetadata === 'teacher') {
        ...
        setTimeout(async () => {
          try {
            ...
            if (!hasTeacher) {
              if (teacherLeftTimeouts.has(roomName)) return;
              ...
              const timeout = setTimeout(async () => {
                try {
                  ...
                  const autoEndRes = await db.query('UPDATE "LiveSession" SET status = \'completed\', "endedAt" = NOW() WHERE "roomId" = $1 AND status = \'live\'', [roomName]);
...
app.post('/api/transcribe', requireClassroomAuth, upload.single('audio'), async (req, res) => {
  try {
    const file = req.file;
...
app.post('/api/doubt', requireClassroomAuth, async (req, res) => {
  ...
  try {
    ...
    segments.sort((a: any, b: any) => a.sessionElapsedMs - b.sessionElapsedMs);
    ...
    const recentSegments = segments.filter((s: any) => (maxElapsedMs - s.sessionElapsedMs) <= last15Mins);

    // Format transcript lines
    const transcriptFormatted = recentSegments.map((s: any) => {
      const min = Math.floor(s.sessionElapsedMs / 60000);
      const sec = Math.floor((s.sessionElapsedMs % 60000) / 1000).toString().padStart(2, '0');
      return `[${min}:${sec}] ${s.name} (${s.role}): ${s.text}`;
    }).join('\n');
...
- Avoid writing raw math or chemical symbols as plain text without $ or $$ wrappers.`;
    ...
    for await (const chunk of geminiRes.body as any) {
...
app.post('/api/summary/update', async (req, res) => {
  ...
  try {
    newSegments.sort((a: any, b: any) => a.sessionElapsedMs - b.sessionElapsedMs);
    const newTranscript = newSegments.map((s: any) => {
      const min = Math.floor(s.sessionElapsedMs / 60000);
      const sec = Math.floor((s.sessionElapsedMs % 60000) / 1000).toString().padStart(2, '0');
      return `[${min}:${sec}] ${s.name} (${s.role}): ${s.text}`;
    }).join('\n');
...
app.post('/api/summary/trigger', requireClassroomAuth, async (req, res) => {
...
app.post('/api/transcript/:sessionId/topic', requireClassroomAuth, requireRole('teacher'), async (req, res) => {
  const { sessionId } = req.params;
